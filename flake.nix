{
  ...
  let
    ...
    # Overlay to guarantee commonly-missing build-system packages exist and have a passthru.format
    # so pyproject-build-systems can inspect them safely.
    fallbackPyprojectOverlay =
      let
        fallbackSpecs = [
          { name = "annotated-types"; format = "pyproject"; defaultPkg = pkgs.python3Packages."annotated-types"; }
          { name = "oldest-supported-numpy"; format = "setuptools"; defaultPkg = pkgs.python3Packages."oldest-supported-numpy"; }
        ];
      in
      final: prev:
        let
          ensurePackage = { name, format, defaultPkg }:
            lib.nameValuePair name (
              let base = (lib.attrByPath [ name ] defaultPkg prev);
              in base.overrideAttrs (old: {
                passthru = (old.passthru or {}) // { format = (old.passthru or {}).format or format; };
              })
            );
        in
          pkgs.lib.listToAttrs (map ensurePackage fallbackSpecs);
